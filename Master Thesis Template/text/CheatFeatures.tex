...
While there are multiple ways to implement cheats—ranging from external overlays to deep internal memory manipulation—we chose a fully \textbf{internal approach} for all features. This includes elements traditionally associated with external overlays, such as ESP and GUI components. In our case, all rendering is performed through \textbf{DirectX 11 hooks} inside the game’s own rendering pipeline, without ever creating a separate overlay window. This internal model ensures minimal performance overhead, seamless integration, and greatly reduces the risk of detection by anti-cheat systems.

Due to the absence of official documentation and the inherently clandestine nature of cheat development, our implementation relied heavily on reverse engineering and iterative experimentation. Instead of injecting disruptive memory modifications or drawing on top of the game window, we opted for subtle and controlled integration directly within CS2’s rendering and logic layers.

We begin our exploration with \textbf{Chams}, one of the most impactful visual modifications in any cheat. Beyond its immediate utility for visibility, it lays the groundwork for more advanced features by exposing exact model data, animation states, and occlusion flags.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{images/chams/chams_diagram.png}
    \caption{Chams Workflow.}
    \label{fig:chams_workflow}
\end{figure}

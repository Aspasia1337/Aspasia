		...
		while (pImportDescriptor->Name) {
			...
			for (; *pThunkRef; pThunkRef++, pFunctRef++) {
				if (IMAGE_SNAP_BY_ORDINAL(*pThunkRef)) {
					*pFunctRef = reinterpret_cast<ULONG_PTR>(_GetProcaddress (hDll, reinterpret_cast<char *>(*pThunkRef & 0xFFFF)));
				}
				else {
					auto *pImport = reinterpret_cast<IMAGE_IMPORT_BY_NAME *>(pBase + (*pThunkRef));
					*pFunctRef = reinterpret_cast<ULONG_PTR>(_GetProcaddress (hDll, pImport->Name));
				}
			}
			pImportDescriptor++;
		}
